...
#define DLL_EXPORT
#endif
// added some unicode definition for Linux probably not all are neccesssary
#ifdef UNICODE 
...
public:
    ...
    bool       Connect(char* pServerAddr, int nPort, int nUDPServerPort = 0, int nMajorVersion = MAJOR_VERSION,
                       int nMinorVersion = MINOR_VERSION, bool bBigEndian = BIG_ENDIAN_RT_PACKET);
    void       Disconnect();
    bool       Connected();
...
private:
    ...
    void AddXMLElementBool(CMarkup* oXML, const _TCHAR* tTag, const bool* pbValue,
                           const _TCHAR* tTrue = ("True"), const _TCHAR* tFalse = ("False"));
    void AddXMLElementBool(CMarkup* oXML, const _TCHAR* tTag, const bool bValue,
                           const _TCHAR* tTrue = ("True"), const _TCHAR* tFalse = ("False"));
    void AddXMLElementInt(CMarkup* oXML, const _TCHAR* tTag, const int* pnValue);
    void AddXMLElementUnsignedInt(CMarkup* oXML, const _TCHAR* tTag, const unsigned int* pnValue);
    void AddXMLElementFloat(CMarkup* oXML, const _TCHAR* tTag, const float* pfValue, unsigned int pnDecimals = 6);
